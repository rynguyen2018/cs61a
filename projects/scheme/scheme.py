...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        return function.eval_call(rest, env)
...
def eval_all(expressions, env):
    ...
    if expressions== nil: 
       return None
    operands= expressions.map(lambda operand: scheme_eval(operand, env))
    final_answer=operands 
    #I wish I could have thought of a better way to do this haha 
    for _ in range(0, len(operands)-1): 
        final_answer= final_answer.second
    return final_answer.first 
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        for _ in range(0,len(formals)):
            child.define(symbol.first, value.first)
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        #if isinstance(self, PrimitiveProcedure):
        parameters= operands.map(lambda operand: scheme_eval(operand, env) )
        return self.apply(parameters, env)
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try: 
            ...
            raise SchemeError("Arguments don't match function bruhhh")
...
class LambdaProcedure(UserDefinedProcedure):
    ...
    def make_call_frame(self, args, env):
        ...
        "*** REPLACE THIS LINE ***"
        # END PROBLEM 12
        child= self.env.make_child_frame(self.formals, args)
        return child
    def __str__(self):
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        check_form(expressions, 2)
        lam_fn= Pair(target.second, expressions.second)
        env.define(target.first, do_lambda_form(lam_fn, env))
        return target.first
    else:
...
def do_and_form(expressions, env):
    ...
    if expressions== nil: 
        return True 
    bool_val= expressions
    for _ in range(0, len(expressions)-1):
        value= scheme_eval(bool_val.first,env) 
        if  value==False and value is not 0:
            return False
        bool_val= bool_val.second
    return scheme_eval(bool_val.first,env)
