...
def find_centroid(cluster):
    ...
    "*** REPLACE THIS LINE ***"
    # END Question 5
    latitiude= [restaurant_location(place)[0] for place in cluster]
    longitude= [restaurant_location(place)[1] for place in cluster]

    return [mean(latitiude), mean(longitude)]
...
def find_predictor(user, restaurants, feature_fn):
    ...
    S_xx= sum([(x_value - mean(xs))**2 for x_value in xs])
    S_yy= sum([(y_value - mean(ys))**2 for y_value in ys])
    S_zip= zip([x - mean(xs) for x in xs], [y - mean(ys) for y in ys])
    S_xy= sum([xy_value[0]* xy_value[1] for xy_value in S_zip])
    # END Question 7
    b= S_xy/S_xx
    a= mean(ys)- b* mean(xs)
...
def best_predictor(user, restaurants, feature_fns):
    ...
    "*** REPLACE THIS LINE ***"
    # END Question 8
    return_values= [list(find_predictor(user, reviewed, fn)) for fn in feature_fns]
    return max(return_values, key= lambda x: x[1])[0]
