...
def deep_map_mut(fn, link):
    ...
    #link= Link(first, deep_map_mut(fn,link.rest))
def two_list(lst1, lst2):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(lst1, lst2, count): 
        if count != 0 : 
            if len(lst2)==1 and lst2[0] !=0:
                return Link(lst1[len(lst1)-1])
            else: 
                return Link(lst1[0], helper(lst1, lst2, count-1))
        else: 
            return helper(lst1[1:], lst2[1:], lst2[1:][0]) 
    return helper(lst1, lst2, lst2[0])
def has_cycle(link):
...
def cumulative_sum(t):
    ...
    def helper(t):
        if t.is_leaf(): 
            ...
            sub_trees= sum([helper(branch) for branch in t.branches]) 
            t.root+= sub_trees
            return t.root
